...
public class Parser {
    ...
    public void prog() {
        ...
        || look.tag == '/' || look.tag == Tag.NUM || look.tag == Tag.ID){
         expr();
         match(Tag.EOF); // per il fine file
       } else if (look.tag == Tag.ASSIGN || look.tag == Tag.PRINT
    ...
    private void expr() {
      if (look.tag == '+' || look.tag == '-' || look.tag == '*'
       || look.tag == '/' || look.tag == Tag.NUM || look.tag == Tag.ID){ // GUIDA
      switch(look.tag){
    ...
    public static void main(String[] args) {
        Lexer lex = new Lexer();
        String path = "/Users/veronicabosso/Desktop/LFT/Lab/5/Prova0.lft"; // il percorso del file da leggere
        try {
